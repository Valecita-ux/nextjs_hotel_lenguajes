...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    ...
    { id: 'habitaciones', label: 'Habitaciones', icon: Bed, href: '/admin/habitaciones' },
    { id: 'operadores', label: 'Operadores', icon: Users, href: '/admin/operadores' },
    { id: 'reportes', label: 'Reportes', icon: BarChart, href: '/admin/reportes' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Top Bar - Colores Admin (Rosa/Coral) */}
      <div className="h-3 bg-gradient-to-r from-pink-600 via-rose-400 to-coral-500"></div>
      ...
      >
        ...
        <div className="flex items-center justify-center p-6 border-b border-gray-200">
          <div className={`flex ${isExpanded ? 'flex-row' : 'flex-col'} items-center space-x-3 transition-all duration-300`}>
            <div className="w-12 h-12 bg-gradient-to-br from-pink-600 to-rose-500 rounded-full flex items-center justify-center p-2 border-2 border-pink-300">
              <Settings className="w-full h-full text-white" />
            </div>
            {isExpanded && (
              <div className="opacity-0 animate-fade-in">
                <p className="font-playfair text-lg font-bold text-gray-800">Panel Admin</p>
                <p className="font-cormorant text-xs text-pink-500 italic">Gestión Total</p>
              </div>
            )}
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-6 space-y-2 overflow-hidden">
          {menuItems.map(({ id, label, icon: Icon, href }) => {
            ...
            return (
              ...
              >
                <Icon className={`flex-shrink-0 transition-all duration-200 ${isExpanded ? 'w-5 h-5' : 'w-6 h-6'} ${active ? 'text-white' : 'text-gray-500 group-hover:text-pink-600'}`} />
                {isExpanded && (
        ...
        <div className="p-4 border-t border-gray-200 space-y-3">
          <div className={`flex items-center ${isExpanded ? 'px-3' : 'justify-center'} transition-all duration-300`}>
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-pink-600 to-rose-500 flex items-center justify-center text-white font-bold flex-shrink-0 font-playfair">
              {getInitials(user?.nombre || 'Admin')}
            </div>
            {isExpanded && (
              <div className="ml-3 opacity-0 animate-fade-in overflow-hidden">
                <p className="font-cormorant text-sm font-semibold text-gray-800 truncate">{user?.nombre || 'Administrador'}</p>
                <p className="font-inter text-xs text-pink-500 truncate">Administrador</p>
              </div>
            )}
          </div>
      ...
      <div className="lg:hidden fixed top-3 left-0 right-0 bg-white shadow-md z-50 border-b border-gray-200 mx-4 rounded-xl">
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-gradient-to-br from-pink-600 to-rose-500 rounded-full flex items-center justify-center p-2">
              <Settings className="w-full h-full text-white" />
            </div>
            <div>
              <p className="font-playfair text-base font-bold text-gray-800">Panel Admin</p>
              <p className="font-cormorant text-xs text-pink-500 italic">Gestión Total</p>
            </div>
          </div>
          <button
            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
            className="p-2 rounded-md text-gray-600 hover:text-pink-600 hover:bg-pink-50"
          >
            ...
            <nav className="p-4 space-y-2">
              ...
              <div className="pt-4 border-t border-gray-200 space-y-3">
                <div className="flex items-center px-4 py-2">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-pink-600 to-rose-500 flex items-center justify-center text-white font-bold font-playfair">
                    {getInitials(user?.nombre || 'Admin')}
                  </div>
                  <div className="ml-3">
                    <p className="font-cormorant text-sm font-semibold text-gray-800">{user?.nombre || 'Administrador'}</p>
                    <p className="font-inter text-xs text-pink-500">Administrador</p>
                  </div>
                </div>
